...
angular.module('app')
    // Chart controller 
   .controller('BasicTableCtrl', ['$scope','$http','urls', function($scope,$http,urls) {
    	
    	$http.get(urls.getAllUsers).success(function(data){
    		...
    		$scope.tabViewData=[];
    		 angular.forEach($scope.allUsers, function (value, key)
    	                {
    			 $scope.tabData=[];
    			 $scope.tabData.push(value.dob);
    	          $scope.tabData.push(value.email) ;
    	          $scope.tabData.push(value.firstName+" "+value.lastName);   
    	          $scope.tabData.push(value.gender) ;
    	          $scope.tabData.push(value.phone)  ;
    	          $scope.tabData.push(value.role)   ;
    	          $scope.tabViewData.push( $scope.tabData);
    	                });
